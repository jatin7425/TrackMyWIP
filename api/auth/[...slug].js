import {
  loginGoogleHandler,
  callbackGoogleHandler,
  checkSessionHandler,
  logoutHandler,
  meHandler
} from '../../lib/authHandlers/index.js';

export default async function handler(req, res) {
  // parse path after /api/auth/
  const urlBase = req.headers.host ? `http://${req.headers.host}` : 'http://localhost';
  const full = new URL(req.url, urlBase);
  const pathname = full.pathname || req.url;
  const parts = pathname.replace(/^\/api\/auth\/?/, '').split('/').filter(Boolean);
  const route = parts.join('/');

  // Dispatch
  try {
  // OTP/signup endpoints removed. Only OAuth/session routes are supported.
    if (route === 'login-google' && req.method === 'GET') return loginGoogleHandler(req, res);
    if (route === 'callback/google' && req.method === 'GET') return callbackGoogleHandler(req, res);
    if (route === 'check-session' && req.method === 'GET') return checkSessionHandler(req, res);
    if (route === 'logout' && req.method === 'POST') return logoutHandler(req, res);
    if (route === 'me' && req.method === 'GET') return meHandler(req, res);

    return res.status(404).json({ message: 'Not found' });
  } catch (err) {
    console.error('Auth dispatcher error:', err);
    return res.status(500).json({ message: 'Server error' });
  }
}
